...
|:-:|:-:|:-:|:-:|:-:|
|Cave Escape|-|-|プルリクエスト|・フォルダーを整理する<br>・実行ファイル名をゲーム名にする<br>・Credits<br>・カタカナにするか英語にするか表記を合わせる|
|ホップホップ|-|-|ESCを連打すると終了確認が複数表示されるのを解消したバージョンに差し替える|・README.txtを用意して、起動方法や操作方法などの基本的な内容を書いておく<br>・ライトの設定をRealtimeにする<br>・タイトルでESCキーでアプリ終了|
|探偵はウイルス|印刷完了|印刷完了|完了|・既存のREADME.txtを作品用のものに差し替え<br>・セーブデータを消しておいたほうがよいかも<br>・config.tjsの;configSaveOverwriteをtrueに設定|
|ReVerser|印刷完了|印刷待ち|完了|・README.txtを追加<br>・最終ステージに進めないことがある<br>・最終ステージのドアを開こうとするとドアの効果音がしたあと、操作不能になる|
|RECAPTURE|不要|不要|不要|完了|
|疾陰の駆者|不要|不要|完了|完了|
|生き残り|不要|不要|不要|完了|
...
### 確認
- RECAPTUREでTabキーを押すと表示されていた操作説明が表示されなくなったことの確認
  - インストカードがあるから不要という話になっていたような
...
- もくもく会方式
  - 開始時に現状と今日やることを簡単に報告
    ...
    - [カタチオクル企画概要書](https://docs.google.com/document/d/1g4hKMjrdqWvZjH7tnUw5zi7cqxxYsuAI91VAvHp6TUU/)
  - 終了時に今日やったことを簡単に報告
- 1週間ゲームジャム「おくる」
...
- プロジェクトの提出
  - Unityの場合
    ...
    - 次の手順でzipファイルを作成して、指定のフォルダーへコピー
      ...
      - デスクトップにコピーしたUnityプロジェクトフォルダーを開いて .gitとBuildsとLibraryフォルダーを削除する
...
## 後期4, 5週目(10/24, 26, 31, 11/2)

### 内容
- 各自、作業報告の記入
- フルスクリーンの場合はESCキーで終了したい
  - ひとまず完成して余力があれば、ESCキーですぐに終了させず、「終了しますか？」のような確認画面を入れる
- タイトル画面などにビルドバージョン(日付など)を表示しておくとよい
- 作業と質疑応答
...
- 進捗をチーム内に共有
  - 確認用の実行ファイルは以下にコピー
    - `X:\2023年\ゲーム学科\卒業制作\実行ファイル`に作品名のフォルダーを作成して、その中に実行ファイルをコピー
...
- チームごとに担当作業をSlackで報告
  - Slackにチーム用のチャンネルを作成
  - メンバーリストを書き、担当した作業を箇条書き

## 14週目(8/29, 31)

### 話題
- [ティラノゲームフェス2023](https://twitter.com/tyranobase/status/1696483454509261033)
  - 8/31参加登録締め切り
  - 9月ブラッシュアップ
  - 10月上旬開幕予定
...
- 31 チーム打ち合わせ
  ...
  - 探偵はウィルス
    - 締め切りに向けて作業
  - Re:Start
    - テーマを「上る」などに変更して、最小構成のゲームを組み立てなおす
    - DATフェスタ後、EXプロジェクトとして企画のテコ入れを検討
  - Re:Verser
    - ゲームクリエイター甲子園への応募完了
    - アイテムの追加、不具合対応など
    - フルスクリーン対応版を提出
...
### 内容
- [ゲームクリエイター甲子園](https://game.creators-guild.com/gck2023/)へのエントリー
  - GCG公式LINEの登録
  - エントリーシートを登録
    - 学部／コース：ゲームプログラムコース
    - 学科：ゲーム
...
- DATフェスタの仕様検討。Googleドキュメントなどに、箇条書きでまとめる
  ...
  - 理想バージョン
  - 進め方
    - やりたいことをすべて書き出す。この時点では実現可能性は考えない
    - 書き出し終えたら、書き出したリストからMVPに必要な項目を選ぶ
...
- ゲームクリエイター甲子園の応募準備(応募するチームのみ)
  ...
  - ソースコードのとりまとめ
  - サムネイル用画像(1920x1080)
  - [ゲームクリエイター甲子園応募作品一覧](https://gameparade.creators-guild.com/works)
...
### 発表用データのコピー先
`Z:\2023\student\sotsusei`内の各作品フォルダー内。
...
- 仕様書の作成
  ...
  - 参考 [よけとる試作版仕様書](https://docs.google.com/spreadsheets/d/16jNh4mKVjusu1bzpv06IbR8abfwVfFfFzKCNs7Byw8g/)
...
- 2コマ目 [内藤時浩. ゲーム制作のWBS分解](https://codeknowledge.livedoor.blog/archives/12980840.html)を参考にWBSを作成
  - 起動からの動作を考えて、必要な状態を大項目として挙げる
  - 各状態について切り替えからの動作を考えて、中項目に分解する
  - ゲーム中などの中項目に更に多数の動作が必要なものについては、小項目に分解する
  - 各状態の仮画面を作成する
    - 手書き、グラフィックツール、PowerPoint、Unityでモックを作るなど手段は問わない
    - スクリーンを表す枠を描き、画面に対してのレイアウトを示すこと
  - 分解しながら、各状態専用の処理、複数の状態で使われる共通処理を挙げる
  - 必要なグラフィックやアニメ、BGM、効果音、フォントなどの素材を挙げる
  - 項目は、一人が１週間以内に完了できそうな規模まで分解する
  - 項目は以下の3種類の優先度を設定する
    - 高：目的達成に必須な項目。当面は試遊版に必要なものと位置付ける。MUST / REQUIRED / SHALLに該当
    - 中：間に合えば入れるべき項目。SHOULD / RECOMMENDEDに該当
    - 低：中項目が入れ終わった後に更に余裕があれば入れることを検討する項目。MAY / OPTIONALに該当

<!--
- [資料の共有URLはこちらへ報告](https://docs.google.com/forms/d/e/1FAIpQLSdPXGSQ3ueVghzfhK7DAGMT4nFAj9RK3wM842cuz9i_LPFzUA/viewform?usp=sf_link)
-->
...
### 内容
- 1日目 企画提案のまとめと共有
  - 各自、春休みに検討してきたチーム企画の案や技術的に調べたことなどをチーム内でとりまとめて、クラスで共有
    - 主題(テーマ)と手段(コンセプト)
    - 試遊版の規模
...
- 2日目 企画構想とWBSの作成
  - 木曜日も作業をして、金曜日にWBSをまとめることを目標に作業
  - 資料はGoogleドキュメント、スライド、スプレッドシートなどまとめやすいものを選ぶ。まずはGoogleドキュメントにまとめて必要に応じて他のツールの利用を考えるのがよさそう
  - 企画が不明瞭なチームは引き続き、企画のテーマとコンセプトの定義
  - テーマとコンセプトがまとまったチームは、[内藤時浩. ゲーム制作のWBS分解](https://codeknowledge.livedoor.blog/archives/12980840.html)を参考にWBSを作成する
    - 参考 [Voxelorer Bird Open版企画構想](https://docs.google.com/document/d/1-6sB9m8iG1kSux1ovdeTRY5WTQ9BcGynHrTI-aQ_GAg/)
